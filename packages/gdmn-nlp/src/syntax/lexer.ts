...
import { RusGender, RusCase, NumeralType } from '../morphology/types';
    ...
    if (!isMorphToken(firstToken) || !(firstToken.word instanceof RusNoun)) {
      ...
      if (!isMorphToken(firstToken) || !(firstToken.word instanceof RusNumeral)) {
        ...
        while (endIdx < parts.length) {
          ...
          if(isMorphToken(parts[endIdx + 1][0]) && (parts[endIdx + 1][0] as IMorphToken).word instanceof RusNumeral) {
            ...
            if (!foundOrinalNumreal) {
              foundOrinalNumreal = nextNumeral.lexeme.type === NumeralType.Ordinal;
            }

            if(nextNumeral.lexeme.value % 1000 !== 0) {
              if(String(numural.lexeme.value).length <= String(nextNumeral.lexeme.value).length
                && nextNumeral.lexeme.type !== NumeralType.Ordinal) {
                  throw new Error(`Invalid structure numeral`);
              }

              if ( Number(numural.lexeme.value) % 10 !== 0 && nextNumeral.lexeme.type !== NumeralType.Ordinal) {
                  throw new Error(`Invalid structure numeral`);
              }
            }
          }
        ...
        if (found && firstParts.length) {
          ...
          let idxON = -1;
          if(foundOrinalNumreal) {
            idxON = parts.findIndex(num => ((num[0] as IMorphToken).word as RusNumeral).lexeme.type === NumeralType.Ordinal)
          }
