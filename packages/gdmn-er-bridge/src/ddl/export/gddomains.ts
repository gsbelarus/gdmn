import {Attribute, EnumAttribute, IAttributeAdapter, StringAttribute} from "gdmn-orm";
import {LName} from "gdmn-internals";

export type createDomainFunc =
  (attributeName: string, lName: LName, adapter?: IAttributeAdapter) => Attribute;
export const gdDomains: { [name: string]: createDomainFunc } = {
  "DGENDER": (name: string, lName: LName, adapter?: IAttributeAdapter) =>
  ...
  "DTYPETRANSPORT": (name: string, lName: LName, adapter?: IAttributeAdapter) =>
    new EnumAttribute({
      name, lName, adapter,
      values: [{value: "C"}, {value: "S"}, {value: "R"}, {value: "O"}, {value: "W"}]
    }),
  "GD_DIPADDRESS": (name: string, lName: LName, adapter?: IAttributeAdapter) =>
    new StringAttribute({
  ...
  "DSTORAGE_DATA_TYPE": (name: string, lName: LName, adapter?: IAttributeAdapter) =>
    new EnumAttribute({
