...
class GDMNSortDialog extends React.Component<IGDMNSortDialogProps, {}> {
  ...
  selectField = (idx: number) => (event: React.FormEvent<IComboBox>, option?: IComboBoxOption, index?: number, value?: string): void => {
    if (!option) return;
  ...
  render() {
    ...
    const fields = () => fieldDefs.map( fd => ({ key: `${fd.fieldName}`, text: fd.caption ? fd.caption : fd.fieldName }) );
        ...
        <div className="GDMNSortDialogContainer">
          {
            sortFields.map( (sf, idx) =>
              <div key={idx} className={cn('GDMNSortDialogRow', { GDMNAttnSortDialogRow: idx >= 0 && idx === attnFieldIdx })}>
                <IconButton iconProps={{ iconName: sf.asc ? 'Ascending' : 'Descending' }} onClick={this.selectFieldOrder(idx)} />
                <IconButton iconProps={{ iconName: sf.groupBy ? 'RowsChild' : 'Table' }} onClick={() => this.onGroupBy(idx)} />
                <IconButton disabled={!sf.groupBy} iconProps={{ iconName: sf.calcAggregates ? 'Calculator' : 'Footer' }} onClick={() => this.onCalcAggregates(idx)} />
                <ComboBox
                ...
                />
                <IconButton disabled={sortFields.length === fieldDefs.length} iconProps={{ iconName: 'Add' }} title="Add" ariaLabel="Add" onClick={() => this.onAdd(idx)} />
                <IconButton disabled={idx < 1} iconProps={{ iconName: 'CaretUpSolid8' }} title="Up" ariaLabel="Up" onClick={() => this.onMoveUp(idx)} />
                <IconButton disabled={idx === sortFields.length - 1} iconProps={{ iconName: 'CaretDownSolid8' }} title="Down" ariaLabel="Down" onClick={() => this.onMoveDown(idx)} />
                <IconButton disabled={sortFields.length === 1} iconProps={{ iconName: 'Delete' }} title="Remove" ariaLabel="Remove" onClick={() => this.onDelete(idx)} />
              </div>
            )
          }
        </div>
