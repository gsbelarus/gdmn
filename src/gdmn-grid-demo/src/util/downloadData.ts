...
fetch(urlNBRBCurrencies)
  ...
  .then( res => fs.writeFileSync('./nbrbcurrencies.json', JSON.stringify(res, undefined, 2)) );

const startDate = new Date(2016, 1, 1);
const endDate = new Date(2018, 8, 1);

const downloadRates = (d: Date, endDate: Date, rates: NBRBRates): Promise<NBRBRates> => {
  if (d < endDate) {
    return fetch(`${urlNBRBRates}?Periodicity=0&onDate=${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`)
      .then( res => res.text() )
...
downloadRates(startDate, endDate, [])
  .then( res => fs.writeFileSync('./nbrbrates.json', JSON.stringify(res, undefined, 2)) )
  .catch( console.log );